...
resultados = {}
for porcentaje_casas in range(1,2 + 1): 
    ...
    G = 171*porcentaje_casas# 0.1% de las casas en la comuna
    ...
    tiempo_ejecucion = fin_temporizador - inicio_temporizador
    resultados[f'{porcentaje_casas/10} %'] = [m.objval, tiempo_ejecucion]

print("\n"+"-"*10+" Manejo Soluciones "+"-"*10)
print(f"El valor objetivo (con datos reales) es de: {resultados}")

with open("resultados.csv",'w',encoding = 'utf-8') as f:
   for key,value in resultados.items():
    f.write(f"{key},{round(value[0],4)},{round(value[1]/60,4)} minutos\n")
# print(f'El tiempo de ejecución fue de {fin_temporizador - inicio_temporizador} segundos')